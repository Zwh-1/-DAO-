...
export function AnonymousClaimForm() {
  ...
  const renderWitnessConfig = useMemo(
    () => (
      <div className="space-y-5">
        ...
        {statusFetchFailed && (
          <div
            className="rounded-md border px-3 py-2 text-sm border-red-200 text-red-800 bg-red-50"
          >
            <p>无法获取活动信息，请稍后重试。</p>
            <button
              type="button"
              className="text-xs mt-1 underline underline-offset-2 text-red-800"
        ...
        <div
            className="space-y-4 pt-1 border-t border-slate-200"
          >
          ...
          <button
            type="button"
            className="text-sm underline-offset-2 hover:underline text-left w-full text-primary"
            onClick={() => {
              setAdvancedOpen((o) => {
                if (!o) {
          ...
          {advancedOpen && (
            <div className="space-y-3 pl-2 border-l-2 border-slate-200">
              ...
              <div>
                ...
                <textarea
                  title="Merkle路径（20个域元素，逗号分隔）"
                  placeholder="请输入20个域元素，用逗号分隔，例如：123456,789012,..."
  ...
  const renderError = useMemo(
    () => (
      <div className="space-y-6">
        <div
          className="border rounded-lg p-6 text-center bg-red-50 border-red-200"
        >
  ...
  return (
    <div className="bg-white rounded-xl shadow-xl max-w-3xl w-full mx-auto overflow-hidden border border-slate-200">
      <div
        className="px-6 py-5 border-b border-slate-800 bg-primary"
      >
      ...
      <div className="px-6 sm:px-8 py-4 bg-slate-50 border-t border-slate-200">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
          ...
          <span className={isConnected ? 'text-success' : 'text-alert'}>
            {isConnected ? '● 已连接' : '● 未连接'}
          </span>
        </div>
      </div>
