 ...
 * 
 * 消息协议：
 * - 输入：{ type: 'GENERATE_PROOF', wasmPath, zkeyPath?, input }
...
self.addEventListener('message', async (event: MessageEvent<WorkerMessage>) => {
  const { type, wasmPath, zkeyPath: explicitZkeyPath, input, taskId } = event.data;
  ...
  if (type === 'GENERATE_PROOF') {
    ...
    try {
      ...
      // 生成证明（优先使用显式 zkeyPath，否则从 wasmPath 推导）
      const zkeyPath = explicitZkeyPath ?? wasmPath!.replace('.wasm', '.zkey');
