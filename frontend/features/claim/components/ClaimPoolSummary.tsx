...
export function ClaimPoolSummary({
  ...
  return (
    <div
      ...
      style={{ borderColor: '#E2E8F0', borderLeftWidth: 4, borderLeftColor: PRIMARY }}
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {tsStart && tsEnd && (
          <span
            ...
            style={{
              color: phaseColor,
              background: phase === 'active' ? '#F0FDF4' : '#FFFBEB',
            }}
          >
      ...
      {contractUnavailable && (
        <p className="text-sm rounded-md border px-3 py-2" style={{ borderColor: '#FDE68A', color: '#92400E', background: '#FFFBEB' }}>
          链上合约未连接或未配置（RPC / ANONYMOUS_CLAIM_ADDRESS）。仍可尝试链下同步 Merkle，但无法从链上读取池子与时间窗。
        </p>
      )}

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        <div className="rounded-md p-2" style={{ background: '#F8FAFC' }}>
          <div style={{ color: '#64748B' }}>池内余额</div>
          <div className="font-semibold mt-0.5 text-slate-900">
            {status?.totalBalance != null ? formatWeiEthLabel(status.totalBalance) : '—'}
          </div>
        </div>
        <div className="rounded-md p-2" style={{ background: '#F8FAFC' }}>
          <div style={{ color: '#64748B' }}>累计已领取</div>
          <div className="font-semibold mt-0.5 text-slate-900">
            {status?.totalClaimed != null ? formatWeiEthLabel(status.totalClaimed) : '—'}
          </div>
        </div>
        <div className="rounded-md p-2" style={{ background: '#F8FAFC' }}>
          <div style={{ color: '#64748B' }}>领取次数</div>
          <div className="font-semibold mt-0.5 text-slate-900">
            {status?.claimCount ?? '—'}
          </div>
        </div>
        <div className="rounded-md p-2" style={{ background: '#F8FAFC' }}>
          <div style={{ color: '#64748B' }}>剩余可领（统计）</div>
          <div className="font-semibold mt-0.5 text-slate-900">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs" style={{ color: '#64748B' }}>
        <div>
