...
import { getDefaultRpcUrlForChain, getNetworkByChainId } from "./wallet/networks";
...
export async function resetExternalWalletProvider(): Promise<void> {
  ...
  unsubscribed = false;
  isInitializing = false;
  const { disconnectWalletConnectSession } = await import("./wallet/walletconnect-session");
