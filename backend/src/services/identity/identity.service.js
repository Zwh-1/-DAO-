...
/**
 * 登录时自动铸造 SBT（若不存在）
 * - 链上模式：跳过（Oracle 才有权限铸造）
 * - 本地/离线模式：用地址数值作为占位 commitment 自动创建
 * @param {string} address 钱包地址
 */
export async function autoMintSBT(address) {
    const key = address.toLowerCase();
    // 本地已有记录，无需重复铸造
    if (_localSbtTokens.has(key)) return;

    // 若合约可用且链上已有 token，不做任何操作（由 Oracle 管理）
    const contracts = getRelayContracts();
    if (contracts) {
        try {
    ...
    // 本地/离线模式：用地址数值作为占位 commitment 自动铸造
    const commitment = String(BigInt(key));
    await mintSBT(key, commitment);
}
...
export async function getSBTInfo(address) {
    ...
    if (contracts) {
        try {
            const tokenId = await contracts.sbt.tokenOf(key);
            if (tokenId === 0n) return { address: key, sbtExists: false, source: "onchain" };
