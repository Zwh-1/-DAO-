...
import { parseGroth16ProofForVault, submitClaimOnchain } from "../../chain/onchain.js";
import {
  insertNullifierDb,
  countNullifiers,
  getPool,
...
/**
 * 配置文件上传（multer）
 * - 存储目录：./uploads/evidence
 * - 文件命名：使用 SHA256 哈希防止冲突
 * - 大小限制：10MB 单文件
 */
...
router.get(
  ...
  requireAuth,
  (req, res) => {
    const address = String(req.auth?.address || "").trim().toLowerCase();
    if (!address) {
      return res.status(401).json({ code: 4010, error: "未登录或 JWT 中无地址" });
    }
...
/**
 * POST /v1/claim/:claimId/evidence
 * 上传理赔证据文件
 * 
 * 权限要求：已认证用户
 * 限流：5 次/分钟
 ...
 * 请求：
 *   - multipart/form-data
 *   - files: 文件数组（最多 5 个，每个 ≤10MB）
 * 
 * 响应：
 *   - evidenceCid: 证据存储标识（文件哈希）
 *   - evidenceUrl: 证据访问 URL
 *   - fileCount: 上传文件数量
 * 
 * 安全约束：
 *   - 文件类型白名单校验
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // 生成证据标识（使用第一个文件的哈希作为 CID）
    const evidenceCid = files[0].filename;
    const evidenceUrl = `/uploads/evidence/${evidenceCid}`;

    // 记录审计日志（脱敏）
    const authed = String(req.auth?.address || "").toLowerCase();
    console.log(
      `[证据上传] claimId=${claimId}, address=${maskAddress(authed)}, fileCount=${files.length}`
    );
